...
def pageParser(filePath: str, projectName: str):
    if "android" in filePath:
        return parserForAndroid(filePath, projectName)
    elif "ofbiz" in filePath or "org.apache.shiro.session" in filePath or "org.antlr.runtime.tree" in filePath:
        return parserForOfBiz(filePath, projectName)
    elif "org.eclipse.jface" in filePath:
        return parserForEclipseJFace(filePath, projectName)
    else:
        return defaultParse(filePath, projectName)

def defaultParse(filePath: str, projectName: str):
    classInfo = {}
    ...
    method_table = soup.find('table', {'summary': 'Method Summary table, listing methods, and an explanation'})
    ...
    for row in method_rows:
        ...
        if len(cols) == 2:
            ...
            method_signature = ''.join((cols[0].code.text.strip() + " " + cols[1].code.text.strip()).split("\n"))
...
def parserForAndroid(filePath: str, projectName: str):
    classInfo = {}
    ...
    # Find the table that contains the method information
    method_table = soup.find('table', {'class': 'responsive methods', 'id':'pubmethods'})
...
def parserForEclipseJFace(filePath: str, projectName: str):
    classInfo = {}
    ...
    for row in method_rows:
        ...
        method_signature = ''.join((a.code.text.strip() + " " + b.code.text.strip()).split("\n"))
        method_signature = re.sub(r'\s+', ' ', method_signature)
        method_signature = re.sub(r'\s*,\s*', ', ', method_signature)
...
def parserForOfBiz(filePath: str, projectName: str):
    classInfo = {}
...
if __name__ == '__main__':
    filePath = "../javadoc/commons-io-2.12.0-javadoc/org/apache/commons/io/output/AbstractByteArrayOutputStream.html"
    pageDoc = pageParser(filePath)
