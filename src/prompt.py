...
memoryOperation = "In Java programs, the typical examples over memory are as follows. You can determine which kinds of memory operations conducted by a method call according to" \
                  "the method name and its specification description. A method might induce multiple operations.\n"

memoryOpExamples = [
    "- memory read: If a method name or its specification description contains the word \"get\" or its synonyms, the method would read data from inner storage.\n",
    "- memory write: If a method name or its specification description contains the word \"set\" or its synonyms, the method would write data to inner storage.\n",
    "- deletion upon memory: If a method name or its specification description contains the word \"remove\" or its synonyms, the method would remove data from inner storage.\n",
    "- insertion upon memory: If a method name or its specification description contains the word \"put\" or its synonyms, the method would insert data to inner storage.\n"
]
...
def getInitialPromptForMemoryOperationType(promptMode: str, magicWords = {}):
    if "manualPrompt" in promptMode:
        ...
        initialPrompt = memoryOperation
        initialPrompt += "- memory read: If a method name or its specification description contains "
        initialPrompt += magicWords["memory read"]
        initialPrompt += " or its synonyms, the method would read data from inner storage.\n"
        ...
        initialPrompt += " or their synonyms, the method would write data to inner storage.\n"
        ...
        initialPrompt += " or their synonyms, the method would remove data from inner storage.\n"
        ...
        initialPrompt += " or their synonyms, the method would insert data to inner storage.\n"
        return initialPrompt
...
def getQuestion(promptMode: str):
    if "manualPrompt" in promptMode:
        ...
        return memoryTypeQuestion_autoPrompt_TwoTypes
    elif "autoPrompt_FourTypes" in promptMode:
