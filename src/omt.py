...
def maximizeMatchingWeight_CHA(graph, ftype, fname, NNSet: set):
    ...
    for i in range(1, len(nodeList1)):
        ...
        flag = 0
        if ftype[(i, 0)] == 0 and isConceptConflictFree(name1i, name20, False, True, NNSet):
            flag = 1
        elif ftype[(i, 0)] > 0 and isConceptCongruence(name1i, name20, False, True, NNSet):
            flag = 1
        if ftype[(i, 0)] == 0 and isConceptConflictFree(name10, name20, True, True, NNSet):
            flag = 1
        elif ftype[(i, 0)] > 0 and isConceptCongruence(name10, name20, True, True, NNSet):
            flag = 1
        if flag == 1:
            solver.add(e[i][0] <= 1)
        else:
        ...
        matchedValue = max(len(computeCommonConcepts(name10, name1i, True, False, NNSet)),
                                  len(computeCommonConcepts(name20, name1i, True, False, NNSet))) * (3 - ftype[(i, 0)])
        for j in range(1, len(nodeList2)):
            (type2j, name2j) = nodeList2[j]
            noMatchedValue = len(computeCommonConcepts(name1i, name2j, False, False, NNSet)) * (3 - ftype[(i, j)])
    ...
    for i in range(1, len(nodeList1)):
        for j in range(1, len(nodeList2)):
            (type1i, name1i) = nodeList1[i]
            (type2j, name2j) = nodeList2[j]
            obj += e[i][j] * len(computeCommonConcepts(name1i, name2j, False, False, NNSet)) * (ftype[(i, j)] + 2)

    (type10, name10) = nodeList1[0]
    (type20, name20) = nodeList2[0]
    len0 = len(computeCommonConcepts(name10, name20, True, True, NNSet))

    for i in range(1, len(nodeList1)):
        (type1i, name1i) = nodeList1[i]
        len1 = len(computeCommonConcepts(name1i, name10, False, True, NNSet))
        len2 = len(computeCommonConcepts(name1i, name20, False, True, NNSet))
...
# Key decision procedure of specification inference (original version: do not consider class hierarchy)
def maximizeMatchingWeight_NoCHA(graph, ftype, fname, NNSet: set):
    ...
    if solver.check() == sat:
        ...
        if target is None:
            ...
            fieldConfidence = max(fieldConfidence, computeNameSimilarity(name1, name2, NNSet, True, True))
            print(name1, name2)
            print("field conf: ", fieldConfidence)
            return str(specVec) + "-->" + str(target), opVal * 1.0 / (1 + len(specVec)), fieldConfidence
    else:
        print("No solution found")
        return None, None